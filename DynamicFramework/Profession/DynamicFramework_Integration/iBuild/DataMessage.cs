...
namespace iBuild
{
    struct DataMessage
    {
        ...
        /// SQL查询表结构
        /// </summary>
        public const string TableSQL = @"SELECT ( CASE WHEN a.colorder = 1 THEN d.name ELSE d.name END ) FieldPageName, f.value AS FieldPageLabel, a.colorder FieldID, a.name FieldName, ( CASE WHEN COLUMNPROPERTY(a.id, a.name, 'IsIdentity') = 1 THEN 'O' ELSE 'X' END ) FieldIdentity, ( CASE WHEN ( SELECT COUNT (*) FROM sysobjects WHERE ( name IN ( SELECT name FROM sysindexes WHERE (id = a.id) AND ( indid IN ( SELECT indid FROM sysindexkeys WHERE (id = a.id) AND ( colid IN ( SELECT colid FROM syscolumns WHERE (id = a.id) AND (name = a.name) ) ) ) ) ) ) AND (xtype = 'PK') ) > 0 THEN 'O' ELSE 'X' END ) FieldPrimary, b.name FieldType, a.length FieldSize, COLUMNPROPERTY(a.id, a.name, 'PRECISION') AS FieldLength, isnull( COLUMNPROPERTY(a.id, a.name, 'Scale'), 0 ) AS FieldDouble, ( CASE WHEN a.isnullable = 1 THEN 'O' ELSE 'X' END ) FieldIsNull, isnull(e. TEXT, '') FieldDefault, isnull(g.[value], ' ') AS FieldLabel FROM syscolumns a LEFT JOIN systypes b ON a.xtype = b.xusertype INNER JOIN sysobjects d ON a.id = d.id AND d.xtype = 'U' AND d.name <> 'dtproperties' LEFT JOIN syscomments e ON a.cdefault = e.id LEFT JOIN sys.extended_properties g ON a.id = g.major_id AND a.colid = g.minor_id LEFT JOIN sys.extended_properties f ON d.id = f.major_id AND f.minor_id = 0 AND f.minor_id = 0 WHERE b.name IS NOT NULL {0} ORDER BY d.name";
